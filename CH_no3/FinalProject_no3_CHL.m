%% ============================================================
% CYCLONE PROJECT: ALFRED vs FINA vs CYCLONE-FREE BASELINE
% CHL + NO3
%
...
ch_period_anom_a  = ch_mean_a  - ch_base_mean;
no3_period_anom_a = no3_mean_a - no3_base_mean;

%% Fina maps
[ch_mean_f,ch_std_f] = spatial_stats_chl(fina_ch);
[no3_mean_f,no3_std_f] = spatial_stats_no3_0_50m(fina_no3,no3_depth_range);

ch_period_anom_f  = ch_mean_f  - ch_base_mean;
no3_period_anom_f = no3_mean_f - no3_base_mean;

%% 3-day peak anomaly maps
ch_peak3_a = peak3_map_chl(alfred_ch,alfred_peak);
no3_peak3_a = peak3_map_no3_0_50m(alfred_no3,alfred_peak,no3_depth_range);

[ch_base_peak3_a,ch_base_peak3_std_a] = baseline_3day_map_chl(baseline_ch_files,alfred_peak);
[no3_base_peak3_a,no3_base_peak3_std_a] = baseline_3day_map_no3_0_50m(baseline_no3_files,alfred_peak,no3_depth_range);
...
[ch_base_peak3_f,ch_base_peak3_std_f] = baseline_3day_map_chl(baseline_ch_files,fina_peak);
[no3_base_peak3_f,no3_base_peak3_std_f] = baseline_3day_map_no3_0_50m(baseline_no3_files,fina_peak,no3_depth_range);

ch_peak3_anom_f  = ch_peak3_f  - ch_base_peak3_f;
no3_peak3_anom_f = no3_peak3_f - no3_base_peak3_f;

%% Significance masks: only retain anomaly > 2*baseline std
ch_peak3_sig_a = ch_peak3_anom_a;
ch_peak3_sig_a(abs(ch_peak3_anom_a) <= 2*ch_base_peak3_std_a) = NaN;

ch_peak3_sig_f = ch_peak3_anom_f;
ch_peak3_sig_f(abs(ch_peak3_anom_f) <= 2*ch_base_peak3_std_f) = NaN;

no3_peak3_sig_a = no3_peak3_anom_a;
no3_peak3_sig_a(abs(no3_peak3_anom_a) <= 2*no3_base_peak3_std_a) = NaN;

no3_peak3_sig_f = no3_peak3_anom_f;
no3_peak3_sig_f(abs(no3_peak3_anom_f) <= 2*no3_base_peak3_std_f) = NaN;
...
ch_anom_abs = percentile_no_toolbox(abs([ch_period_anom_a(:); ch_period_anom_f(:); ch_peak3_sig_a(:); ch_peak3_sig_f(:)]),98);
no3_anom_abs = percentile_no_toolbox(abs([no3_period_anom_a(:); no3_period_anom_f(:); no3_peak3_sig_a(:); no3_peak3_sig_f(:)]),98);
...
%% Anomaly figures
plot_anomaly_comparison(ch_period_anom_a,ch_peak3_sig_a,ch_period_anom_f,ch_peak3_sig_f, ...
    lat_ch,lon_ch,lat_box,lon_box, ...
    'CHL Anomaly Comparison', ...
    'CHL_Anomaly_Comparison', ...
    'mg m^{-3}', output_dir, clims.ch_anom);

plot_anomaly_comparison(no3_period_anom_a,no3_peak3_sig_a,no3_period_anom_f,no3_peak3_sig_f, ...
    lat_no3,lon_no3,lat_box,lon_box, ...
    'NO_3 0-50 m Anomaly Comparison', ...
    'NO3_0_50m_Anomaly_Comparison', ...
...
function [base3,base3_std] = baseline_3day_map_chl(files,event_date)
...
for i = 1:length(files)
    ...
    if any(keep)
        all_maps = cat(3,all_maps,data(:,:,keep));
    end

end

base3 = mean(all_maps,3,'omitnan');
base3_std = std(all_maps,0,3,'omitnan');

end

function [base3,base3_std] = baseline_3day_map_no3_0_50m(files,event_date,depth_range)
...
function plot_anomaly_comparison(period_a,peak3_a,period_f,peak3_f,lat,lon,lat_box,lon_box,title_str,filename,var_label,output_dir,anom_lim)
...
nexttile
plot_single_map(period_a,lat,lon,lat_box,lon_box,cmap,land_color,anom_lim)
title('Alfred Period Anomaly')
cb = colorbar; cb.Label.String = var_label;

nexttile
plot_single_map(peak3_a,lat,lon,lat_box,lon_box,cmap,nonsig_color,anom_lim)
title('Alfred 3-day anomaly (significant only)')
cb = colorbar; cb.Label.String = var_label;

nexttile
plot_single_map(period_f,lat,lon,lat_box,lon_box,cmap,land_color,anom_lim)
title('Fina Period Anomaly')
cb = colorbar; cb.Label.String = var_label;

nexttile
plot_single_map(peak3_f,lat,lon,lat_box,lon_box,cmap,nonsig_color,anom_lim)
title('Fina 3-day anomaly (significant only)')
cb = colorbar; cb.Label.String = var_label;
