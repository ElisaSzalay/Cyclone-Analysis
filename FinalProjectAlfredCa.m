...
C_a= double(ncread(filename,'CHL'));  % mass concentration of chlorophyll a in seawater


%% Reduce dimensions for 2D variables
C_a = squeeze(C_a); % remove singleton dimension if only one depth layer

%% Compute mean and standard deviation over time (3rd dimension)
C_a_mean     = mean(C_a, 3, 'omitnan');   % mean ignoring NaNs
C_a_std     = std(C_a, 0, 3, 'omitnan');   % standard deviation ignoring NaNs
...
% Create 2D matrix
C_a_mean = reshape(C_a_mean,length(lon),length(lat));
C_a_std  = reshape(C_a_std ,length(lon),length(lat));
...
title('Mean Surface Chlorophyll-a - Cyclone Alfred')
exportgraphics(gcf, fullfile(outputFolder, 'Mean_Surface_Chlorophyll-a_Alfred.png'));
%% STD map
...
title('STD Surface Chlorophyll-a - Cyclone Alfred')
exportgraphics(gcf, fullfile(outputFolder, 'STD_Surface_Chlorophyll-a_Alfred.png'));